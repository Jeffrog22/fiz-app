...
export async function gerarFrequenciaXLSX(
  ...
  const { data: logs, error: logsError } = await supabase
    ...
    .select('*')
    .eq('tenant_id', tenantId)
    .gte('data', diasLetivos[0] || `${ano}-${String(mes).padStart(2,'0')}-01`)
    .lte('data', diasLetivos[diasLetivos.length - 1] || `${ano}-${String(mes).padStart(2,'0')}-31`);
  ...
  for (const turma of turmas) {
    ...
    const labelExtenso: Record<string, string> = { 'Seg/Ter': 'Segunda e Terça', 'Seg/Qua': 'Segunda e Quarta', 'Seg/Qui': 'Segunda e Quinta', 'Seg/Sex': 'Segunda e Sexta', 'Ter/Qua': 'Terça e Quarta', 'Ter/Qui': 'Terça e Quinta', 'Ter/Sex': 'Terça e Sexta', 'Qua/Qui': 'Quarta e Quinta', 'Qua/Sex': 'Quarta e Sexta', 'Qui/Sex': 'Quinta e Sexta', 'Seg': 'Segunda', 'Ter': 'Terça', 'Qua': 'Quarta', 'Qui': 'Quinta', 'Sex': 'Sexta', 'Sab': 'Sábado', 'Seg/Ter/Qua': 'Segunda, Terça e Quarta', 'Seg/Ter/Qui': 'Segunda, Terça e Quinta', 'Seg/Qua/Sex': 'Segunda, Quarta e Sexta', 'Ter/Qua/Qui': 'Terça, Quarta e Quinta', 'Ter/Qua/Sex': 'Terça, Quarta e Sexta', 'Qua/Qui/Sex': 'Quarta, Quinta e Sexta', 'Seg/Ter/Qua/Qui': 'Segunda a Quinta', 'Seg a Sex': 'Segunda a Sexta', };
