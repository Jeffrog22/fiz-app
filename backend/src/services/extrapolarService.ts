...
/**
 * Extrapola status 'cancelado' para TODOS os ?ndices de uma data (compromete_dia).
 *
 * Usado quando cardAula sugere AULA_CANCELADA por condi??es clim?ticas/piscina.
 * Aplica status 'cancelado' para todos os alunos ativos em TODOS os ?ndices (0 a maxIndices-1).
 *
 * Regras:
 * - N?o sobrescreve registros com origem = 'manual' ou 'calendario'
 * - N?o sobrescreve registros j? cancelados (j? ? o que queremos)
 * - Cria/atualiza registros com status = 'cancelado', origem = 'extrapolado'
 *
 * @returns quantidade de logs criados/atualizados
 */
export async function extrapolarCancelamento(
  ...
  if (!data) throw new AppError('Campo data ? obrigat?rio', 400);

  // Aplica para todos os ?ndices do dia
  const indicesAProcessar: number[] = [];
  ...
  if (indicesAProcessar.length === 0) {
    return { message: 'Nenhum ?ndice para cancelar', count: 0 };
  }
  ...
  if (alunosError) {
    console.error('[extrapolarCancelamento] Erro ao buscar alunos:', alunosError.message);
    throw new AppError('Erro ao buscar alunos', 500);
  }
  ...
  if (alunoIds.length === 0) {
    return { message: 'Nenhum aluno ativo para cancelar', count: 0 };
  }
  ...
  if (fetchError) {
    console.error('[extrapolarCancelamento] Erro ao buscar logs:', fetchError.message);
    throw new AppError('Erro ao verificar chamadas existentes', 500);
  }
  ...
  // Para cada ?ndice, aplica 'cancelado' para cada aluno, respeitando preced?ncia
  for (const idx of indicesAProcessar) {
    for (const alunoId of alunoIds) {
      const key = `${alunoId}_${idx}`;
      const existing = existingMap.get(key);
      ...
      if (existing) {
        // N?o sobrescrever se for manual ou calendario
        if (existing.origem === 'manual' || existing.origem === 'calendario') {
          continue;
        }
        // J? est? cancelado
  ...
  if (logsCriados.length === 0) {
    return { message: 'Nenhum ?ndice precisou de cancelamento', count: 0 };
  }
  ...
  for (let i = 0; i < logsCriados.length; i += BATCH_SIZE) {
    ...
    if (insertError) {
      console.error('[extrapolarCancelamento] Erro ao inserir batch:', insertError.message, 'idx:', i);
      for (const log of batch) {
        const { error: singleError } = await supabase
          .from('chamadas_log')
          .upsert(log, { onConflict: 'tenant_id,data,grupo_id,indice_aula' });
        if (singleError) {
          console.error('[extrapolarCancelamento] Erro ao inserir individual:', singleError.message);
        }
      }
    }
  }
  ...
  return { message: `Cancelamento extrapolado para ${logsCriados.length} registros`, count: logsCriados.length };
}
