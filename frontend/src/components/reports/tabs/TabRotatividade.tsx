...
const TabRotatividade: React.FC = () => {
  ...
  const totalMatriculas = hasData ? data.reduce((s, d) => s + d.matricula_inicial, 0) : 0;
  const totalTransferencias = hasData ? data.reduce((s, d) => s + d.transferencia, 0) : 0;
  const totalCorrecoes = hasData ? data.reduce((s, d) => s + d.correcao, 0) : 0;
