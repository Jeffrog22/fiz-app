...
const TabFrequenciaAluno: React.FC = () => {
  ...
  const mediaPresenca = hasData ? Math.round(data.reduce((s, d) => s + d.percentual_presenca, 0) / data.length) : 0;
  const top = hasData ? data.reduce((best, d) => d.percentual_presenca > best.percentual_presenca ? d : best, data[0]) : null;
  const bottom = hasData ? data.reduce((worst, d) => d.percentual_presenca < worst.percentual_presenca ? d : worst, data[0]) : null;
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <CardStat titulo="Maior Presença" valor={top ? `${top.percentual_presenca}%` : '-'} cor="text-primary-600" icon="🏆" />
            <CardStat titulo="Menor Presença" valor={bottom ? `${bottom.percentual_presenca}%` : '-'} cor="text-red-600" icon="⚠️" />
          </div>
