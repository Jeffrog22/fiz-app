...
const TabFrequenciaTurma: React.FC = () => {
  ...
  const melhor = hasData ? data.reduce((best, d) => d.percentual_presenca > best.percentual_presenca ? d : best, data[0]) : null;
  const pior = hasData ? data.reduce((worst, d) => d.percentual_presenca < worst.percentual_presenca ? d : worst, data[0]) : null;
